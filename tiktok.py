...
def parse_relative_date(rel_date_str, current_date=None):
    """
    rel_date_str: 예) "방금 전", "18시간 전", "1일 전", "2주 전" 등의 문자열 또는 "2-1"과 같이 월-일만 있는 경우
    current_date: 기준 날짜 (지정하지 않으면 datetime.now() 사용)
    반환: current_date를 기준으로 계산한 "YYYY-MM-DD" 문자열
    """
...
# Seleninum webdriver 크롬으로
def initialize_driver():
    chrome_options = Options()
        # 브라우저 허용 (근데 알림 설정인듯 2로 해도 상관없는듯. 보안 문제 x)
    chrome_options.add_experimental_option("prefs", {
        "profile.default_content_setting_values.notifications": 1  # 1: 허용, 2: 차단
    })
    # chrome_options.add_argument("--headless")  # 브라우저 창 열고 실행해야할듯 봇 탐지 퍼즐 있음
    chrome_options.add_argument("--disable-gpu")
    chrome_options.add_argument("--no-sandbox")
    
    # 만약 ChromeDriver 경로를 직접 지정해야 하면 아래 주석처리 된거로 수정하여 사용
...
# slow_scroll 함수 추가: 페이지를 여러 단계로 나누어 스크롤합니다.
...
def get_video_info(driver, video_url):
    ...
    # 업로드 날짜가 상대 표현일 경우 실제 날짜로 변환 (예: "1일 전" → "2025-02-09")
